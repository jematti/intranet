...
if (isset($_POST['disable'])) {
    ...
    $action = "disable";
    $entity = "storage";
    $details = "Se deshabilitó el documento con ID $store_id";

    // Actualizar el estado del documento en la tabla `storage`
    $query = "UPDATE storage SET status = 0 WHERE store_id = ?";
    ...
    if ($stmt->execute()) {
        ...
        if ($stmt_audit->execute()) {
            echo json_encode(['status' => 'success', 'message' => 'Documento deshabilitado correctamente.']);
        } else {
        ...
        echo json_encode(['status' => 'error', 'message' => 'Error al deshabilitar el documento.']);
    }
